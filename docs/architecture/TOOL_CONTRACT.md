...
| 73 | `symbol_search` | READ | yes | no |
| 74 | `find_definition` | READ | yes | no |
| 75 | `find_references` | READ | yes | no |
| 76 | `find_implementations` | READ | yes | no |
| 77 | `call_hierarchy` | READ | yes | no |
| 78 | `import_graph` | READ | yes | no |
| 79 | `dependency_graph` | READ | yes | no |
| 80 | `module_graph` | READ | yes | no |
| 81 | `type_search` | READ | yes | no |
| 82 | `trace_symbol` | READ | yes | no |
| 83 | `context_ranking` | READ | yes | no |
| 84 | `debug_context` | READ | yes | no |
| 85 | `review_context` | READ | yes | no |
| 86 | `change_context` | READ | yes | no |
| 87 | `symbol_context` | READ | yes | no |
| 88 | `test_context` | READ | yes | no |
| 89 | `dependency_context` | READ | yes | no |
| 90 | `git_context` | READ | yes | no |
| 91 | `frontend_context` | READ | yes | no |
| 92 | `backend_context` | READ | yes | no |
| 93 | `route_intent` | READ | yes | no |
| 94 | `recipe_list` | READ | yes | no |
| 95 | `recipe_describe` | READ | yes | no |
| 96 | `recipe_run` | EXECUTE | no | no |
| 97 | `dry_run` | READ | yes | no |
| 98 | `review_changes` | READ | yes | no |
| 99 | `changed_symbols` | READ | yes | no |
| 100 | `affected_modules` | READ | yes | no |
| 101 | `git_history_context` | READ | yes | no |
| 102 | `git_blame_context` | READ | yes | no |
| 103 | `discover_tests` | READ | yes | no |
| 104 | `run_affected_tests` | EXECUTE | no | no |
| 105 | `test_failures` | READ | yes | no |
| 106 | `coverage_context` | READ | yes | no |
| 107 | `test_history` | READ | yes | no |
| 108 | `cache_stats` | READ | yes | no |
| 109 | `cache_clear` | WRITE | no | no |
| 110 | `cache_invalidate` | WRITE | no | no |
| 111 | `hook_list` | READ | yes | no |
| 112 | `hook_register` | WRITE | no | no |
| 113 | `hook_remove` | WRITE | no | no |
| 114 | `skill_match` | READ | yes | no |
| 115 | `skill_load` | READ | yes | no |
| 116 | `plugin_install` | DANGEROUS | no | yes |
| 117 | `plugin_list` | READ | yes | no |
| 118 | `plugin_enable` | WRITE | no | no |
| 119 | `plugin_disable` | WRITE | no | no |
| 120 | `plugin_remove` | DANGEROUS | no | yes |
| 121 | `session_context` | READ | yes | no |
| 122 | `session_checkpoint` | WRITE | no | no |
| 123 | `session_resume` | READ | yes | no |
| 124 | `session_history` | READ | yes | no |
| 125 | `response_mode` | READ | yes | no |
| 126 | `inspect_web_app` | READ | yes | no |
| 127 | `debug_ui` | READ | yes | no |
| 128 | `capture_ui_state` | READ | yes | no |
| 129 | `form_context` | READ | yes | no |
| 130 | `network_context` | READ | yes | no |
| 131 | `console_context` | READ | yes | no |
| 132 | `browser_debug_context` | READ | yes | no |
| 133 | `windows_environment` | READ | yes | no |
| 134 | `service_context` | READ | yes | no |
| 135 | `process_context` | READ | yes | no |
| 136 | `port_context` | READ | yes | no |
| 137 | `registry_context` | READ | yes | no |
| 138 | `event_log_context` | READ | yes | no |
| 139 | `installed_runtime_context` | READ | yes | no |
| 140 | `path_context` | READ | yes | no |
| 141 | `startup_context` | READ | yes | no |
| 142 | `mcp_discover` | READ | yes | no |
| 143 | `mcp_health` | READ | yes | no |
| 144 | `mcp_resources` | READ | yes | no |
| 145 | `task_create` | EXECUTE | no | no |
| 146 | `task_status` | READ | yes | no |
| 147 | `task_cancel` | EXECUTE | no | no |
| 148 | `task_result` | READ | yes | no |
| 149 | `task_list` | READ | yes | no |
| 150 | `delegate` | EXECUTE | no | no |
| 151 | `delegate_status` | READ | yes | no |
| 152 | `delegate_cancel` | EXECUTE | no | no |
| 153 | `delegate_result` | READ | yes | no |
| 154 | `parallel_delegate` | EXECUTE | no | no |
| 155 | `permission_check` | READ | yes | no |
| 156 | `permission_profile` | READ | yes | no |
| 157 | `live_logs_query` | READ | yes | no |
| 158 | `live_logs_status` | READ | yes | no |
| 159 | `telemetry_dashboard` | READ | yes | no |
| 160 | `context_economy_stats` | READ | yes | no |
| 161 | `execution_plan` | READ | yes | no |
| 162 | `repo_map` | READ | yes | no |
| 163 | `context_expand` | READ | yes | no |
| 164 | `recovery_status` | READ | yes | no |
| 165 | `tool_schema_list` | READ | yes | no |
| 166 | `tool_schema_register` | WRITE | no | no |
| 167 | `capabilities` | READ | yes | no |
| 168 | `tool_search` | READ | yes | no |
| 169 | `tool_dynamic_filter` | READ | yes | no |
| 170 | `tool_describe` | READ | yes | no |
| 171 | `tool_categories` | READ | yes | no |
| 172 | `tool_function_find` | READ | yes | no |
| 173 | `tool_aliases` | READ | yes | no |
| 174 | `mcp_hub` | READ | yes | no |
| 175 | `dev_context` | READ | yes | no |
| 176 | `recipe_catalog` | READ | yes | no |
| 177 | `capture_screenshot` | READ | yes | no |
| 178 | `compare_screenshot` | READ | yes | no |
| 179 | `dom_snapshot` | READ | yes | no |
| 180 | `layout_metadata` | READ | yes | no |
| 181 | `visual_context` | READ | yes | no |
| 182 | `inspect_workbook` | READ | yes | no |
| 183 | `compare_workbook_layout` | READ | yes | no |
| 184 | `render_excel_preview` | READ | yes | no |
| 185 | `inspect_pdf` | READ | yes | no |
| 186 | `compare_pdf_pages` | READ | yes | no |
| 187 | `project_profile_get` | READ | yes | no |
| 188 | `project_profile_set` | WRITE | no | no |
| 189 | `handoff_context` | READ | yes | no |
| 190 | `benchmark_run` | EXECUTE | no | no |
| 191 | `regression_report` | READ | yes | no |
| 192 | `sandbox_exec` | EXECUTE | no | no |
| 193 | `event_watch` | EXECUTE | no | no |
| 194 | `crash_trace` | READ | yes | no |
| 195 | `lsp_diagnostics` | READ | yes | no |
| 196 | `lsp_rename` | WRITE | no | no |
| 197 | `debug_attach` | EXECUTE | no | no |
| 198 | `debug_step` | EXECUTE | no | no |
| 199 | `git_worktree_spawn` | DANGEROUS | no | yes |
| 200 | `db_inspect` | READ | yes | no |
| 201 | `db_query` | DANGEROUS | no | yes |
| 202 | `office_ppt` | DANGEROUS | no | yes |
| 203 | `office_outlook` | DANGEROUS | no | yes |
| 204 | `pdf_extract_tables` | READ | yes | no |
| 205 | `docx_merge` | WRITE | no | no |
