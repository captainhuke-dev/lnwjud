...
This block is generated from the built `ToolRegistry`. Current count: **214 tools**.
...
| 46 | `notification` | EXECUTE | no | no |
| 47 | `file_dialog` | EXECUTE | yes | no |
| 48 | `clipboard` | DANGEROUS | no | no |
| 49 | `web_fetch` | DANGEROUS | yes | no |
| 50 | `audio` | DANGEROUS | no | no |
| 51 | `screen_record` | DANGEROUS | no | no |
| 52 | `office` | DANGEROUS | no | no |
| 53 | `scheduler` | DANGEROUS | no | yes |
| 54 | `wsl_exec` | EXECUTE | no | yes |
| 55 | `wsl_fs` | READ | yes | no |
| 56 | `skills_list` | DANGEROUS | no | yes |
| 57 | `skills_read` | DANGEROUS | no | yes |
| 58 | `mcp_list` | DANGEROUS | no | yes |
| 59 | `mcp_describe` | DANGEROUS | no | yes |
| 60 | `mcp_call` | DANGEROUS | no | yes |
| 61 | `workspace_context` | READ | yes | no |
| 62 | `workspace_context_continue` | READ | yes | no |
| 63 | `workspace_full_scan` | READ | yes | no |
| 64 | `workspace_full_scan_continue` | READ | yes | no |
| 65 | `workspace_snapshot` | READ | yes | no |
| 66 | `search_all` | READ | yes | no |
| 67 | `read_many_files` | READ | yes | no |
| 68 | `read_file_page` | READ | yes | no |
| 69 | `read_file_page_continue` | READ | yes | no |
| 70 | `workspace_index` | READ | yes | no |
| 71 | `workspace_index_status` | READ | yes | no |
| 72 | `workspace_index_watch` | READ | yes | no |
| 73 | `workspace_index_stop` | READ | yes | no |
| 74 | `session_handoff` | READ | yes | no |
| 75 | `verify_incremental` | EXECUTE | no | no |
| 76 | `symbol_search` | READ | yes | no |
| 77 | `find_definition` | READ | yes | no |
| 78 | `find_references` | READ | yes | no |
| 79 | `find_implementations` | READ | yes | no |
| 80 | `call_hierarchy` | READ | yes | no |
| 81 | `import_graph` | READ | yes | no |
| 82 | `dependency_graph` | READ | yes | no |
| 83 | `module_graph` | READ | yes | no |
| 84 | `type_search` | READ | yes | no |
| 85 | `trace_symbol` | READ | yes | no |
| 86 | `context_ranking` | READ | yes | no |
| 87 | `debug_context` | READ | yes | no |
| 88 | `review_context` | READ | yes | no |
| 89 | `change_context` | READ | yes | no |
| 90 | `symbol_context` | READ | yes | no |
| 91 | `test_context` | READ | yes | no |
| 92 | `dependency_context` | READ | yes | no |
| 93 | `git_context` | READ | yes | no |
| 94 | `frontend_context` | READ | yes | no |
| 95 | `backend_context` | READ | yes | no |
| 96 | `route_intent` | READ | yes | no |
| 97 | `recipe_list` | READ | yes | no |
| 98 | `recipe_describe` | READ | yes | no |
| 99 | `recipe_run` | EXECUTE | no | no |
| 100 | `dry_run` | READ | yes | no |
| 101 | `review_changes` | READ | yes | no |
| 102 | `changed_symbols` | READ | yes | no |
| 103 | `affected_modules` | READ | yes | no |
| 104 | `git_history_context` | READ | yes | no |
| 105 | `git_blame_context` | READ | yes | no |
| 106 | `discover_tests` | READ | yes | no |
| 107 | `run_affected_tests` | EXECUTE | no | no |
| 108 | `test_failures` | READ | yes | no |
| 109 | `coverage_context` | READ | yes | no |
| 110 | `test_history` | READ | yes | no |
| 111 | `cache_stats` | READ | yes | no |
| 112 | `cache_clear` | WRITE | no | no |
| 113 | `cache_invalidate` | WRITE | no | no |
| 114 | `hook_list` | READ | yes | no |
| 115 | `hook_register` | WRITE | no | no |
| 116 | `hook_remove` | WRITE | no | no |
| 117 | `skill_match` | READ | yes | no |
| 118 | `skill_load` | READ | yes | no |
| 119 | `plugin_install` | DANGEROUS | no | yes |
| 120 | `plugin_list` | READ | yes | no |
| 121 | `plugin_enable` | WRITE | no | no |
| 122 | `plugin_disable` | WRITE | no | no |
| 123 | `plugin_remove` | DANGEROUS | no | yes |
| 124 | `session_context` | READ | yes | no |
| 125 | `session_checkpoint` | WRITE | no | no |
| 126 | `session_resume` | READ | yes | no |
| 127 | `session_history` | READ | yes | no |
| 128 | `response_mode` | READ | yes | no |
| 129 | `inspect_web_app` | READ | yes | no |
| 130 | `debug_ui` | READ | yes | no |
| 131 | `capture_ui_state` | READ | yes | no |
| 132 | `form_context` | READ | yes | no |
| 133 | `network_context` | READ | yes | no |
| 134 | `console_context` | READ | yes | no |
| 135 | `browser_debug_context` | READ | yes | no |
| 136 | `windows_environment` | READ | yes | no |
| 137 | `service_context` | READ | yes | no |
| 138 | `process_context` | READ | yes | no |
| 139 | `port_context` | READ | yes | no |
| 140 | `registry_context` | READ | yes | no |
| 141 | `event_log_context` | READ | yes | no |
| 142 | `installed_runtime_context` | READ | yes | no |
| 143 | `path_context` | READ | yes | no |
| 144 | `startup_context` | READ | yes | no |
| 145 | `mcp_discover` | READ | yes | no |
| 146 | `mcp_health` | READ | yes | no |
| 147 | `mcp_resources` | READ | yes | no |
| 148 | `task_create` | EXECUTE | no | no |
| 149 | `task_status` | READ | yes | no |
| 150 | `task_cancel` | EXECUTE | no | no |
| 151 | `task_result` | READ | yes | no |
| 152 | `task_list` | READ | yes | no |
| 153 | `delegate` | EXECUTE | no | no |
| 154 | `delegate_status` | READ | yes | no |
| 155 | `delegate_cancel` | EXECUTE | no | no |
| 156 | `delegate_result` | READ | yes | no |
| 157 | `parallel_delegate` | EXECUTE | no | no |
| 158 | `permission_check` | READ | yes | no |
| 159 | `permission_profile` | READ | yes | no |
| 160 | `live_logs_query` | READ | yes | no |
| 161 | `live_logs_status` | READ | yes | no |
| 162 | `telemetry_dashboard` | READ | yes | no |
| 163 | `context_economy_stats` | READ | yes | no |
| 164 | `execution_plan` | READ | yes | no |
| 165 | `repo_map` | READ | yes | no |
| 166 | `context_expand` | READ | yes | no |
| 167 | `recovery_status` | READ | yes | no |
| 168 | `tool_schema_list` | READ | yes | no |
| 169 | `tool_schema_register` | WRITE | no | no |
| 170 | `capabilities` | READ | yes | no |
| 171 | `tool_search` | READ | yes | no |
| 172 | `tool_dynamic_filter` | READ | yes | no |
| 173 | `tool_describe` | READ | yes | no |
| 174 | `tool_categories` | READ | yes | no |
| 175 | `tool_function_find` | READ | yes | no |
| 176 | `tool_aliases` | READ | yes | no |
| 177 | `mcp_hub` | READ | yes | no |
| 178 | `dev_context` | READ | yes | no |
| 179 | `recipe_catalog` | READ | yes | no |
| 180 | `capture_screenshot` | READ | yes | no |
| 181 | `compare_screenshot` | READ | yes | no |
| 182 | `dom_snapshot` | READ | yes | no |
| 183 | `layout_metadata` | READ | yes | no |
| 184 | `visual_context` | READ | yes | no |
| 185 | `inspect_workbook` | READ | yes | no |
| 186 | `compare_workbook_layout` | READ | yes | no |
| 187 | `render_excel_preview` | READ | yes | no |
| 188 | `inspect_pdf` | READ | yes | no |
| 189 | `compare_pdf_pages` | READ | yes | no |
| 190 | `project_profile_get` | READ | yes | no |
| 191 | `project_profile_set` | WRITE | no | no |
| 192 | `handoff_context` | READ | yes | no |
| 193 | `benchmark_run` | EXECUTE | no | no |
| 194 | `regression_report` | READ | yes | no |
| 195 | `sandbox_exec` | EXECUTE | no | no |
| 196 | `event_watch` | EXECUTE | no | no |
| 197 | `crash_trace` | READ | yes | no |
| 198 | `lsp_diagnostics` | READ | yes | no |
| 199 | `lsp_rename` | WRITE | no | no |
| 200 | `debug_attach` | EXECUTE | no | no |
| 201 | `debug_step` | EXECUTE | no | no |
| 202 | `git_worktree_spawn` | DANGEROUS | no | yes |
| 203 | `git_worktree_remove` | DANGEROUS | no | yes |
| 204 | `db_inspect` | READ | yes | no |
| 205 | `db_query` | DANGEROUS | no | yes |
| 206 | `office_ppt` | DANGEROUS | no | yes |
| 207 | `office_outlook` | READ | yes | no |
| 208 | `pdf_extract_tables` | READ | yes | no |
| 209 | `docx_merge` | WRITE | no | no |
| 210 | `self_heal_plan` | READ | yes | no |
| 211 | `self_heal_apply` | DANGEROUS | no | yes |
| 212 | `skills_import` | WRITE | no | no |
| 213 | `agent_swarm_run` | EXECUTE | no | no |
| 214 | `tool_batch` | DANGEROUS | no | yes |
...
- `skills_*` and `mcp_*` remain full-access bridge tools and do not silently
  flatten child-server tools into the 214-tool configurable catalog.
